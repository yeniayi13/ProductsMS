...
builder.Services.AddScoped(sp =>
{
    ...
    return dbContext.Database.GetCollection<CategoryEntity>("Categories"); // Nombre de la colecci�n en MongoDB
});

builder.Services.AddScoped(sp =>
{
    var dbContext = sp.GetRequiredService<IApplicationDbContextMongo>();
    return dbContext.Database.GetCollection<ProductEntity>("Products"); // Nombre de la colecci�n en MongoDB
});
...
builder.Services.AddSingleton<IConnectionRabbbitMQ>(provider =>
{
    ...
    rabbitMQConnection.InitializeAsync().Wait(); // ?? Ejecutar inicializaci�n antes de inyectarlo
...
// Iniciar el consumidor autom�ticamente con `RabbitMQBackgroundService`
builder.Services.AddHostedService<RabbitMQBackgroundService>();
